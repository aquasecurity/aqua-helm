...
| `securityContextConstraints.create`    | OpenShift only. Whether the chart should render an SCC for the scanner. Set to `false` when the SCC is managed externally (e.g., by aqua-operator)           | `true`                 | `NO`                                                                      |
| `securityContextConstraints.name`      | OpenShift only. Name of the SCC the chart creates (only used when `create=true`). Renames the chart-managed SCC; does not adopt an existing SCC              | `{{ .Release.Name }}-scc` | `NO`                                                                   |
