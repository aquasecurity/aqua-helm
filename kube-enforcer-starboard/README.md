...
| `starboard.OPERATOR_TARGET_NAMESPACES`  | This determine the install mode, which in turn determines the multitenancy support of the operator | `(blank)` | `YES` <br> `(blank string)=> ALLNAMESPACES, foo,bar.baz => specific NAMESPACES` |
...
| `starboard.OPERATOR_SCAN_JOB_RETRY_AFTER`  | The duration to wait before retrying a failed scan job | `30s` | `YES` |
...
| `starboard.OPERATOR_HEALTH_PROBE_BIND_ADDRESS`  | The TCP address to bind to for serving health probes, i.e. /healthz/ and /readyz/ endpoints | `:9090` | `true` |
| `starboard.OPERATOR_CIS_KUBERNETES_BENCHMARK_ENABLED`  |  The flag to enable CIS Kubernetes Benchmark scanner | `true` | `YES` |
...
| `starboard.OPERATOR_BATCH_DELETE_DELAY`  | The duration to wait before deleting another batch of config audit reports | `10s` | `YES` |
